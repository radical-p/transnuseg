...
def main():
    ...
    parser.add_argument(
        ...
        required=True,
        default=0.3,
        help="coeffiecient of the weight of nuclei mask loss",
    )
    parser.add_argument(
        "--beta",
        required=True,
        default=0.35,
        help="coeffiecient of the weight of normal edge loss",
    )
    parser.add_argument(
        "--gamma",
        required=True,
        default=0.35,
        help="coeffiecient of the weight of cluster edge loss",
    )
    parser.add_argument(
        "--sharing_ratio",
        required=True,
        default=0.5,
        help=" ratio of sharing proportion of decoders",
    )
    ...
    parser.add_argument(
        "--dataset", required=True, default="Histology", help="Histology, Radiology"
    )
    ...
    parser.add_argument(
        "--model_path", default=None, help="the path to the pretrained model"
    )
    ...
    create_dir("./log")
    logging.basicConfig(
        filename="./log/log.txt".format(model_type, dataset, str(now)),
        level=logging.INFO,
    ...
    logging.info(
        "Batch size : {} , epoch num: {}, alph: {}, beta : {}, gamma: {}, sharing_ratio = {}".format(
            batch_size, num_epoch, alpha, beta, gamma, sharing_ratio
        )
    )
    print(
        "Batch size : {} , epoch num: {}, alph: {}, beta : {}, gamma: {}, sharing_ratio = {}".format(
    ...
    if dataset == "Radiology":
        ...
        total_data = MyDataset(dir_path=data_path, transform=train_transforms)
        train_set_size = int(len(total_data) * 0.8)
        test_set_size = len(total_data) - train_set_size
    ...
    for epoch in range(num_epoch):
        ...
        for phase in ["train", "test"]:
            ...
            for i, d in enumerate(dataloaders[phase]):
                ...
                loss_seg = 0.4 * ce_loss1(
                    output1, semantic_seg_mask.long()
                ) + 0.6 * dice_loss1(output1, semantic_seg_mask.float(), softmax=True)
                loss_nor = 0.4 * ce_loss2(
                    output2, normal_edge_mask.long()
                ) + 0.6 * dice_loss2(output2, normal_edge_mask.float(), softmax=True)
                loss_clu = 0.4 * ce_loss3(
                    output3, cluster_edge_mask.long()
                ) + 0.6 * dice_loss3(output3, cluster_edge_mask.float(), softmax=True)
                loss_dice = dice_loss1(output1, semantic_seg_mask.float(), softmax=True)
                ...
                running_loss_wo_dis += (
                    alpha * loss_seg + beta * loss_nor + gamma * loss_clu
                ).item()  ## Loss without distillation loss
                running_loss_seg += loss_seg.item()  ## Loss for nuclei segmantation
                running_dice_seg += loss_dice.item()  ## Loss for nuclei segmantation
                if phase == "train":
                    optimizer.zero_grad()
                    loss.backward()
                    optimizer.step()
            ...
            epoch_loss_wo_dis = (
                running_loss_wo_dis / dataset_sizes[phase]
            )  ## Epoch Loss without distillation loss
            epoch_loss_seg = (
                running_loss_seg / dataset_sizes[phase]
            )  ## Epoch Loss for nuclei segmantation
            epoch_dice = (
                running_dice_seg / dataset_sizes[phase]
            )  ## Epoch dice for nuclei segmantation
            logging.info(
                "Epoch {},: loss {}, {},time {}".format(
                    epoch + 1, epoch_loss, phase, e - s
                )
            )
            logging.info(
                "Epoch {},: loss without distillation {}, {},time {}".format(
                    epoch + 1, epoch_loss_wo_dis, phase, e - s
                )
            )
            logging.info(
                "Epoch {},: loss seg {}, {},time {},newloss {}".format(
                    epoch + 1, epoch_loss_seg, phase, e - s, epoch_dice
                )
            )
            ...
            if phase == "test" and epoch_loss_seg < best_loss:
                ...
                logging.info(
                    "Best val loss {} save at epoch {}".format(best_loss, epoch + 1)
                )
    ...
    torch.save(
        best_model_wts,
        "./saved/model_epoch:{}_testloss:{}_{}.pt".format(
            best_epoch, best_loss, str(now)
        ),
    )
    logging.info(
        "Model saved. at {}".format(
            "./saved/model_spoch:{}_testloss:{}_{}.pt".format(
                best_epoch, best_loss, str(now)
            )
        )
    )
    ...
    with torch.no_grad():
        for i, d in enumerate(testloader, 0):
            ...
            # addres = "".join(["/content/Ali/", str(count) + ".png"])
            # addres2 = "".join(["/content/Ali/", "mask" + str(count) + ".png"])

            # cv2.imwrite(addres, output1)
            # cv2.imwrite(addres2, semantic_seg_mask)
